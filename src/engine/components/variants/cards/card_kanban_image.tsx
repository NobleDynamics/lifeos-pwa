/**
 * CardKanbanImage Variant Component
 * 
 * A Kanban card with a cover image at the top, plus text details, priority indicator,
 ...
 * 
 * @module engine/components/variants/cards/card_kanban_image
 */
...
/**
 * CardKanbanImage - Kanban card with cover image and text details.
 * 
 * Structure:
 * ┌─────────────────────────────────────┐
 ...
 * - media: Cover image URL (optional)
 * - media_height: CSS class for image height (default: 'h-24')
 ...
 */
export function CardKanbanImage({ node }: VariantComponentProps) {
  ...
  const mediaHeight = useSlot<string>('media_height', 'h-24')
  ...
  return (
    <div
      ...
      data-variant="card_kanban_image"
      data-node-id={node.id}
      {...longPressHandlers}
    >
      {/* Cover Image */}
      {media ? (
        <div className={cn("w-full overflow-hidden bg-dark-300", mediaHeight)}>
          <img
            src={media}
            alt={headline}
        ...
        <div className={cn(
          "w-full overflow-hidden bg-dark-300",
